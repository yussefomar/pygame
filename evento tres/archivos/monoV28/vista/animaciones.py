ANIMACIONES_FONDO = {0: {"x": 12, "y": 345, "sprite_ancho": 230, "sprite_alto": 272}}

ANIMACIONES_MARIO = {
    "izquierda": {
        "frames": {
            0: {"x": 16, "y": 203, "sprite_ancho": 12, "sprite_alto": 18},
            1: {"x": 32, "y": 203, "sprite_ancho": 18, "sprite_alto": 18},
            2: {"x": 52, "y": 203, "sprite_ancho": 18, "sprite_alto": 18},
        },
        "frame_delay": 8,
    },
    "arriba": {
        "frames": {
            0: {"x": 72, "y": 203, "sprite_ancho": 12, "sprite_alto": 18},
            1: {"x": 91, "y": 203, "sprite_ancho": 12, "sprite_alto": 18},
            2: {"x": 110, "y": 203, "sprite_ancho": 12, "sprite_alto": 18},
            3: {"x": 132, "y": 203, "sprite_ancho": 12, "sprite_alto": 18},
        },
        "frame_delay": 12,
    },
   "salto": {
        "frames": {0: {"x": 72, "y": 203, "sprite_ancho": 12, "sprite_alto": 18}},
        "frame_delay": 12,
    },
    "muerto": {
        "frames": {0: {"x": 174, "y": 203, "sprite_ancho": 23, "sprite_alto": 21}},
        "frame_delay": 5,
    },
    "reposo": {
        "frames": {0: {"x": 16, "y": 203, "sprite_ancho": 12, "sprite_alto": 18}},
        "frame_delay": 20,
    },
    "golpe": {
        "frames": {
            0: {"x": 12, "y": 225, "sprite_ancho": 20, "sprite_alto": 40},
            1: {"x": 38, "y": 225, "sprite_ancho": 30, "sprite_alto": 40},
            3: {"x": 64, "y": 225, "sprite_ancho": 26, "sprite_alto": 40},
            2: {"x": 86, "y": 225, "sprite_ancho": 38, "sprite_alto": 40},
        },
        "frame_delay": 15,
    },
}

ANIMACIONES_DONKEYKONG = {
    "pecho": {
        "frames": {
            0: {"x": 120, "y": 50, "sprite_ancho": 42, "sprite_alto": 40},
            1: {"x": 175, "y": 50, "sprite_ancho": 42, "sprite_alto": 40},
        },
        "frame_delay": 22,
    },
    "lanza_barril": {
        "frames": {
            0: {"x": 10, "y": 146, "sprite_ancho": 43, "sprite_alto": 40},
            1: {"x": 65, "y": 146, "sprite_ancho": 43, "sprite_alto": 40},
        },
        "frame_delay": 22,
    },
}

ANIMACIONES_FUEGO = {
    "izquierda": {
        "frames": {  # 238 242
            0: {"x": 238, "y": 100, "sprite_ancho": 14, "sprite_alto": 15},
            1: {"x": 242, "y": 100, "sprite_ancho": 14, "sprite_alto": 14},
            2: {"x": 275, "y": 100, "sprite_ancho": 14, "sprite_alto": 14},
        },
        "frame_delay": 15,
    },
    "reposo": {
        "frames": {0: {"x": 10, "y": 50, "sprite_ancho": 42, "sprite_alto": 40}},
        "frame_delay": 10,
    },
}

ANIMACIONES_BARRILES = {0: {"x": 268, "y": 275, "sprite_ancho": 20, "sprite_alto": 35}}

ANIMACIONES_BLOQUE = {0: {"x": 324, "y": 250, "sprite_ancho": 15, "sprite_alto": 8}}
