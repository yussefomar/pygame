...
class Mario(Personaje):
    ...
    def actualizar_posicion(self):
        ...
        if teclas[pygame.K_SPACE]:  # Tecla de golpe
            self.animacion_actual = "golpe"
            self.actualizar_frame()
            en_movimiento = True
        
          # Iniciar salto con la flecha arriba (solo si no se está saltando)
        if teclas[pygame.K_UP] and not self.saltando:
            self.saltando = True
            self.jump_velocity = -15  # Velocidad inicial negativa para subir
            self.animacion_actual = "salto"
            en_movimiento = True

        # Actualizar la física del salto si se está saltando
        if self.saltando:
